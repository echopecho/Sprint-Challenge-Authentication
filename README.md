...
1. What is the purpose of using _sessions_?
    ...
    Sessions allow the users information to be stored on the server.  This persistance provides a better user experience as the user is not required to login on each request.

1. What does bcrypt do to help us store passwords in a secure manner.

  bcrypt hashes the password to an encrypted string so the password isn't simply stored in its string value.

1. What does bcrypt do to slow down attackers?

  bcrypt will re-encrypt the password a multitude of times.  The library takes in an integer and runs the encryption a number of times equal to 2 to the power of the integer provided.
